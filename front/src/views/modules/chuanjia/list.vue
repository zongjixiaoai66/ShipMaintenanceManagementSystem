        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('chuanjia','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanboweixiu/upload/chuanjiaMuBan.xls"
                        >批量导入船家数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('chuanjia','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="chuanjiaUploadSuccess"
                                :on-error="chuanjiaUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('chuanjia','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "chuanjia.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="chuanjiaIdNumber"
                                   header-align="center"
                                   label="船家身份证号">
                        <template slot-scope="scope">
                            {{scope.row.chuanjiaIdNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('chuanjia','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('chuanjia','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('chuanjia','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('chuanjia','修改')" type="success" size="mini" @click="jinqiyong(scope.row.id,scope.row.jinyongTypes)">{{scope.row.jinyongTypes == 1?'禁用账户':'启用账户'}}</el-button>

                            <el-button v-if="isAuth('chuanjia','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                username : null,
                password : null,
                chuanjiaUuidNumber : null,
                chuanjiaName : null,
                chuanjiaPhone : null,
                chuanjiaIdNumber : null,
                chuanjiaPhoto : null,
                sexTypes : null,
                chuanjiaEmail : null,
                jinyongTypes : null,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'chuanjia',//当前表表名,
                        sumColum : 'chuanjia_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'chuanjia_name',//分组字符串字段
                        // types : 'chuanjia_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['chuanjiaDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "chuanjia/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            chuanjiaUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "chuanjia/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            jinqiyong(id,jinyongTypes) {
                ...
                _this.$confirm(msg, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"}).then(() => {
                    _this.$http({url: "chuanjia/update",method: "post",data: {id:id,jinyongTypes:jinyongTypes,}}).then(({data}) => {
                        if(data && data.code === 0){
